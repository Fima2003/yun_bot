...
class UserService:
    async def is_new_user(
        self, user_id: int, chat_id: int, context: ContextTypes.DEFAULT_TYPE
    ) -> bool:
        """
        ...
        try:
            ...
            if time_diff.days > NEW_USER_THRESHOLD_DAYS:
                ...
                logger.info(
                    f"User {user_id} passed threshold ({time_diff.days} days). Marking safe."
                )
